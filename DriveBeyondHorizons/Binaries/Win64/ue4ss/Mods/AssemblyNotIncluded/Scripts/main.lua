...
local polish_probe_baseline = nil
local menu_actions = {}
...
local function open_menu()
    ...
    if not valid(menu_widget) then
        for _, path in ipairs({
            ...
            menu_widget = create_widget(path)
            if valid(menu_widget) then
        ...
        if valid(object) then
            ...
            if id_ok and not existing[tostring(id)] then
                if make_infinite then
                    set_if_present(object, "Quantity", 999999)
                    set_if_present(object, "MaxQuantity", 999999)
                    pcall(function() object:OnRep_Quantity() end)
                end
                changed = changed + 1
            end
        end
...
local function set_unlimited_tools(mode)
    local names
    if mode == "brushes" then
        names = {"RustBrush_C", "PolishBrush_C"}
    ...
    if not valid(speedometer_widget) then
        ...
        if not valid(speedometer_widget) then
            for _, path in ipairs({
                ...
                speedometer_widget = create_widget(path)
                if valid(speedometer_widget) then
                    log("Speedometer loaded using fallback path: " .. tostring(path))
                    break
                end
            end
        end
...
local function update_all_dyn_materials(
    object,
    key,
    scalar,
    add_to_value,
    ...
    for _, vector in ipairs(vectors) do
        local ok, err = call_if_present(
            ...
            true,
            add_to_value == true,
            tonumber(polish) or 1.0)
        if ok then return true end

        ok, err = pcall(function()
            object:UpdateAllDynMaterials(
                material_key(key),
                vector,
                scalar,
                true,
        ...
        if update_all_dyn_log_budget > 0 and err then
            log("UpdateAllDynMaterials failed for " ..
                tostring(key) .. ": " .. tostring(err))
            update_all_dyn_log_budget = update_all_dyn_log_budget - 1
        end
    end
...
local function probe_struct_field(value, names)
    if value == nil then return nil end
    for _, name in ipairs(names) do
        local ok, field = pcall(function() return value[name] end)
        if ok and field ~= nil then return unwrap_hook_value(field) end
    end
    return nil
end

local function probe_scalar_text(value)
    value = unwrap_hook_value(value)
    local number = tonumber(value)
    if number ~= nil then return string.format("%.9g", number) end
    if type(value) == "boolean" then return tostring(value) end
    return object_text(value)
end

local function capture_dyn_entry(out, prefix, explicit_key, entry)
    local key = explicit_key
    if key == nil or type(key) == "number" then
        key = probe_struct_field(entry, {
            "Key_2_9A8C438B4224A6B1657E3997CF43EB5B",
            "Key",
            "ParameterName",
        })
    ...
    local scalar = probe_struct_field(entry, {
        "ScalarParameter_8_52C9880A475C4002B11BC8A2E153EE81",
        "ScalarParameter",
        "Scalar",
        "Value",
    })
    local is_scalar = probe_struct_field(entry, {
        "IsScalarParameter_11_EA81612E4CFE7ABB1E2351A611997EA1",
        "IsScalarParameter",
    })
    local key_text = normalized_material_key(key)
    if key_text == "" then return false end
    out[prefix .. ":" .. key_text] =
        "scalar=" .. probe_scalar_text(scalar) ..
        ";is_scalar=" .. probe_scalar_text(is_scalar)
    return true
end

local function capture_polish_probe(objects)
    local values = {}
    local dyn_seen = {}
    local dyn_count = 0
    local entry_count = 0
    for _, object in ipairs(objects or {}) do
        for _, dyn in ipairs(collect_surface_dyn_components(object)) do
            local identity = unreal_object_id(dyn)
            if identity and not dyn_seen[identity] then
                dyn_seen[identity] = true
                dyn_count = dyn_count + 1
                for _, property in ipairs({"MapDynParam", "ListDynParam"}) do
                    local ok, container =
                        pcall(function() return dyn[property] end)
                    if ok and container then
                        pcall(function()
                            container:ForEach(function(key, entry)
                                if capture_dyn_entry(
                                    values,
                                    identity .. ":" .. property,
                                    unwrap_hook_value(key),
                                    unwrap_hook_value(entry)) then
                                    entry_count = entry_count + 1
                                elseif capture_dyn_entry(
                                    values,
                                    identity .. ":" .. property,
                                    nil,
                                    unwrap_hook_value(key)) then
                                    entry_count = entry_count + 1
                                end
                            end)
                        end)
                ...
                for _, property in ipairs({
                    ...
                    local value = numeric_property(dyn, property)
                    if value ~= nil then
                        values[identity .. ":" .. property] =
                            probe_scalar_text(value)
                    end
                end
            end
        end
    end
    return values, dyn_count, entry_count
end

local function toggle_polish_probe_snapshot()
    if polish_probe_baseline == nil then
        local vehicle = get_current_vehicle()
        if not valid(vehicle) then
            show_message(
                "Enter the target vehicle before capturing the Polish baseline.")
            return
        end
        local objects = vehicle_surface_actors(vehicle)
        local values, dyn_count, entry_count =
            capture_polish_probe(objects)
        polish_probe_baseline = {
            objects = objects,
            values = values,
        }
        log("POLISH_PROBE baseline: objects=" .. tostring(#objects) ..
            ", dyn_components=" .. tostring(dyn_count) ..
            ", entries=" .. tostring(entry_count) .. ".")
        show_message(
            "Polish baseline captured. Exit, polish several surfaces, then press F6.")
        return
    end

    local after, dyn_count, entry_count =
        capture_polish_probe(polish_probe_baseline.objects)
    local changes = {}
    for key, before_value in pairs(polish_probe_baseline.values) do
        local after_value = after[key]
        if after_value ~= nil and after_value ~= before_value then
    ...
    for key, after_value in pairs(after) do
        if polish_probe_baseline.values[key] == nil then
            table.insert(changes, {
                key = key,
                before = "<missing>",
                after = after_value,
    ...
    log("POLISH_PROBE comparison: changes=" .. tostring(#changes) ..
        ", dyn_components=" .. tostring(dyn_count) ..
        ", entries=" .. tostring(entry_count) .. ".")
    for index, change in ipairs(changes) do
        if index > 300 then
            log("POLISH_PROBE output limited to 300 changes.")
            break
        end
        log("POLISH_PROBE_CHANGE " .. change.key .. ": " ..
            change.before .. " -> " .. change.after)
    end
    show_message(
        "Polish comparison captured: " .. tostring(#changes) ..
        " changed value(s).")
    polish_probe_baseline = nil
end
...
local function register(name, callback)
    ...
    RegisterCustomEvent("AO_" .. name, function()
...
register("InfiniteBrushes", function() set_unlimited_tools("brushes") end)
...
RegisterKeyBind(Key.F6, function()
    ExecuteInGameThread(toggle_polish_probe_snapshot)
    return false
end)
